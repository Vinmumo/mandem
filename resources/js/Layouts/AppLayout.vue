...
const nav=[['Dashboard','/'],['Live','/live'],['Standings','/standings'],['Managers','/managers'],['Insights','/stats'],['Hall','/hall-of-fame']];
...
<nav class="mx-auto hidden items-center gap-1 md:flex"><Link v-for="n in nav" :key="n[0]" :href="n[1]" class="rounded-full px-4 py-2 text-sm font-semibold" :class="page.url===n[1]?'bg-[#163f2e] text-white dark:bg-[#f4d35e] dark:text-[#17241d]':'text-[#65756d] hover:bg-black/5 dark:text-[#9cada5] dark:hover:bg-white/5'">{{n[0]}}</Link></nav>
